 ...
 *
 * Padrão esperado para títulos (linhas que funcionam como cabeçalho):
 *  - Enunciado      → linha iniciada com número + ponto  (ex.: "1." ou "12.")
 *  - Alternativas   → linha iniciada com letra maiúscula + ")  (ex.: "A)" "B)")
 *  - Seções em caps → ex.: "ENUNCIADO", "ALTERNATIVAS", "GABARITO", "HABILIDADE"
 *
 * Erros detectados:
 *  - Alternativas fora da ordem A→B→C→D
 *  - Alternativas com letras minúsculas  (ex.: "a)")
 *  - Seções de cabeçalho com formatação mista (ex.: "Gabarito:" ao invés de "GABARITO:")
 */
function validateTitles(raw) {
  const errors = [];
  const lines = raw.split(/\r?\n/);

  // Detecta alternativas presentes e valida ordem e capitalização
  const altPattern    = /^([A-Za-z])\)/;
  const altUpper      = /^[A-D]\)/;
  const altLower      = /^[a-z]\)/;
  const expectedOrder = ['A', 'B', 'C', 'D'];
  let altIndex = 0;

  // Palavras-chave de seção que devem aparecer em MAIÚSCULAS
  const sectionKeywords = [
    'enunciado', 'alternativas', 'gabarito', 'habilidade',
    'competência', 'dificuldade', 'objeto de conhecimento'
  ];
  ...
  lines.forEach((line, i) => {
    ...
    // Verifica alternativas com letra minúscula
    if (altLower.test(trimmed)) {
      const letter = trimmed[0];
      errors.push(`Linha ${lineNum}: alternativa com letra minúscula "${letter})" — use "${letter.toUpperCase()})"`);
    }

    // Verifica ordem das alternativas
    if (altUpper.test(trimmed)) {
      const letter = trimmed[0];
      const expected = expectedOrder[altIndex];
      if (expected && letter !== expected) {
        errors.push(`Linha ${lineNum}: alternativa "${letter})" fora de ordem — esperado "${expected})"`);
      }
      altIndex++;
    }

    // Verifica palavras de seção em formatação mista (não totalmente maiúsculas)
    for (const kw of sectionKeywords) {
      const kwNorm = kw.toLowerCase();
      const lineNorm = trimmed.toLowerCase();
      if (lineNorm.startsWith(kwNorm)) {
        // A linha começa com a palavra-chave; verifica se está em CAPS
        const segment = trimmed.slice(0, kw.length);
        if (segment !== segment.toUpperCase()) {
          errors.push(`Linha ${lineNum}: seção "${segment}" deve estar em MAIÚSCULAS (ex.: "${kw.toUpperCase()}")`);
        }
        break;
      }
    }
